 ...
 * prod build + strict CSP); the E2E suite makes it a standing gate.
 *
 * Unlike razors-edge, atrium has NO tracked/known CSP violation: the one risk —
 * Zod v4's JIT `new Function` probe reaching the client bundle via
 * `site-nav -> header/mobile-nav` — was closed with `z.config({ jitless: true })`
 * (atrium AGENT_NOTES, Phase 3). So every CSP violation here is UNEXPECTED and
 * fails the assertion; there is no allow-list.
 *
...
const VIOLATION_BINDING = '__atriumReportCspViolation';
