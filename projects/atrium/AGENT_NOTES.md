...
## designer-critic — polish-pass VERIFICATION (live site, 2026-06-11)

Verified the cohesion+polish pass on the DEPLOYED site; full verdict in `docs/verify-polish.md`. All five `critique-polish.md` leads CLEARED, no regressions, CSP/console-clean. Recurring patterns for future milestones (so the frontend-engineer can pre-empt, not re-fix):

- **"One component renders N items" is fine ONLY if the component varies its composition off the data.** B-01 was the core not-cohesive cause: one bay template recoloured six times read as one screen, not six showcases. The fix that worked: a deterministic layout selector off the ordinal (left / right-mirrored / centred) + per-room light direction, all bound by ONE shared system (type rhythm, hue language, ruled stack, wow-note). Cohesion = shared system + intentional variation, NEVER sameness. Carry this rule to any future "sequence of cards/rooms" surface.
- **A signature-hue device must FLOOD the surface, not halo it.** B-02/D-10: radial gradients clinging to the top edge with transparent falloff read as decorative trim over a black void. The bathe needs an ambient full-surface tint + a floor pool that rises behind the type. Judge any "lit in its colour" surface by whether the hue reaches the CONTENT band, not just the frame edge.
- **A scrubbed pinned timeline under a fixed CSS perspective front-loads visually.** B-03/D-13: equal scroll late in a translateZ camera moves far fewer pixels than early (perspective foreshortening), so a mathematically-continuous tween still LOOKS frozen in the back third. Remedy that shipped: eased Z + column spread + floor bloom as a second beat. Residual: it is now adequate but gentle — if a future pass wants drama, steepen the Z ease (power2.in) or shorten the pin, OR add a literal new element entering in the back third. Always match perceived choreography length to pin length, measured by PIXELS-moved not scrub-value.
- **The light theme must be authored per-surface, not derived.** B-04 recurred because light was a token re-tune of a dark composition. Every atmospheric layer (hero shaft, bay wash, descent hall) needs a light comp built in its own right (heavier ink, a visible warm core, hues re-keyed as floor washes that survive a cream ground). Build + judge the light theme of each new surface from day one, separately.
- **Kinetic type needs a gesture the eye can SEE at display scale.** D-15 is the one PARTIAL: a weight settle + hue-to-resolved is legible but reads closer to a tinted fade than craft. For a true Aristide-Benoist-class resolve, commit to a clip/mask wipe with enough travel to register, or a weight/width settle large enough to read as motion — a subtle variable-font delta disappears at this size.

## frontend-engineer — craft-residuals pass: N-01 + N-02 closed (2026-06-12)

The two OPTIONAL LOW residuals the live-site verification logged (`docs/verify-polish.md`: N-01 descent back-third "still gentle"; N-02 D-15 title resolve short of a committed clip wipe) are CLOSED. Surgical — four source files + one new capture driver; inside every ADR/hard constraint; the five cleared blockers did NOT regress and directory/about were not touched. Frames in `docs/craft-residuals-shots/` (26: a 6-step title-resolve ladder + a 5-step descent back-third, dark+light, + hero/three-layout regression spot-checks).

### RESIDUAL 2 / N-02 — D-15: the committed title clip WIPE (the technique)

- **A THIRD layered real-text copy of the bay title** — `data-bay-title-edge` in `project-bay.tsx`, the word in the BRIGHT `var(--bay)` hue with a hue textShadow, `aria-hidden`, INVISIBLE at the resting frame (`globals.css` `[data-bay-title-edge] { opacity:0; visibility:hidden }`). It is the travelling WIPE EDGE: the sequence reveals it (`autoAlpha 1`), sweeps its `clip-path` across the word with the title front, then fades it out as it reaches the trailing edge — a bright leading edge the eye locks onto as MOTION at display scale (the gesture the tinted settle lacked).
- **The ghost + final + edge now share ONE wipe axis/direction** (`bays-sequence.tsx` `buildBayTimeline`). Previously the ghost wiped on `fromLeft` while the final's clip was decoupled, so the two read as a tint crossfade. Now: the final starts FULLY clipped along the wipe axis (`clipHidden`) and opens to `clipOpen`; the ghost's "off" clip is the COMPLEMENT (it closes to the trailing edge wherever the final is still clipped) — so the hue retreats exactly as the legible title arrives, one shared front. The 320→600 `wght` + opsz 100→144 settle rides under the wipe (layered, not replaced).
- **Per-LAYOUT wipe axis (D-16 consistency).** `cycle = i % 3` (the SAME ordinal the `compositionFor` layout selector uses): left/right wall bays wipe HORIZONTALLY in their `fromLeft` reveal direction; the centred-specimen bays (cycle 2 = razors-edge/atlas) wipe VERTICALLY (a curtain rising, `inset(...100%...)` from the bottom). So the wipe reads WITH each room's composition.
- **Reduced-motion / no-JS:** the edge copy is hidden by the CSS resting rule and GSAP only sets/sweeps it inside the armed `(min-width:768px)` / `(max-width:767px)` no-preference branches — the `reduced` branch creates nothing. VERIFIED: all six edge bands `visibility:hidden`/`opacity:0` at rest, all six `<h2>` finals resolved (clip none, opacity 1) — nothing frozen mid-wipe.
- **GOTCHA for the next editor:** the edge is a THIRD copy of the same word but is NOT in the a11y tree (`aria-hidden`) — the `<h2>` final remains the sole accessible title (one word to a screen reader). Do not give the edge an id or remove its `aria-hidden`. It must stay invisible at rest (the CSS rule) so the no-JS/reduced floor is the calm composed frame; GSAP's inline `autoAlpha` (visibility+opacity) is what reveals it, and inline beats the stylesheet, so the resting hide is safe.

### RESIDUAL 1 / N-01 — B-03 back-third: the camera change (only the back third)

- **Camera-Z ease `power2.in → power3.in`** (`hero-descent.tsx`). The front+middle of the curve stay gentle (good, untouched); the steeper cubic accel spends clearly MORE of the Z pixel-travel LATE, so the perspective-foreshortened back third reads as the camera rushing into the hall instead of drifting. To keep the steeper curve from netting LESS total late motion, **camZ reach lifted** (desktop 900→1000, mobile 620→720).
- **Pin trimmed further** so there is no soft tail: desktop 1.0→0.85vh, mobile 0.8→0.7vh. The pin now ends right at the perceived end of the choreography.
- **Hand-off second beat strengthened to OWN the back third** (the new event where the camera delta is smallest): starts 0.56 (was 0.62), `yPercent 40→64` travel + a `scale 1→1.08` grow, and the doorway element itself made taller/brighter (`h-[34vh]→h-[42vh]`, inner bloom `h-40→h-56`). So the lit threshold visibly sweeps up into frame across the last third.
- **Floor-pool bloom kept OPACITY-ONLY.** The floor (`[data-colonnade-floor]`) owns a CSS-3D `rotateX(80deg)` placement — a GSAP `scale`/translate decomposes the matrix and clobbers the rotateX (the exact trap `colonnade.tsx` warns about for the columns). I tried a floor scale-up and reverted it; the back-third strengthening is carried by the camera accel + the (2D, transform-safe) hand-off instead.
- **Verified by RENDERED FRAME DELTA, not the tween trace** (the lesson from the prior B-03 regression): `docs/craft-residuals-shots/descent-backthird-0{1,3,5}-dark.png` are clearly distinct — 03 the camera is much deeper + the hand-off doorway rising, 05 the hall fully resolved + bay-1 ("tape") arriving beneath with the doorway at peak. No soft tail.

### Verification (PRODUCTION build, not next dev)

`pnpm -F atrium-web build` (clean; the EPERM standalone-symlink warning is the documented Windows-only benign trace; home First Load JS 149 kB, unchanged) + `next start` :3080, headless chromium via `e2e/verify-polish.mjs`: 7 pin-spacers desktop; 0 CSP violations; 0 console/page errors; 0 non-favicon req failures; reduced-motion = 0 pins + all six titles resolved + all six edge bands hidden; 320px scrollW==clientW. `typecheck` + `lint` clean (project + ROOT strict eslint on the four touched files, exit 0). Vitest 54/54. Playwright 20/20 (harness-owned flipped-GITHUB_BASE webServer — let it own the build, no BASE_URL; a hand-started non-flipped build fails the 7 repo-link specs, the documented env-baking seam, NOT a regression).
