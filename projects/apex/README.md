...
- **ADR-001** — Stack flavour `web-only` (no backend), the R3F + GSAP two-library posture, sovereign light-canonical design tokens, the fully client-side mocked reservation architecture, and the portfolio-composition note (apex is the second web-only project; the three api-heavy backend slots are already filled).
...
- **Lighthouse** (production build, median of 5). Accessibility 96, Best Practices 96, and SEO 100 clear the bar on `/` across both profiles. Desktop `/` Performance is 97 and `/reserve` is 99. The **mobile** `/` Performance is below 95 on the Lighthouse mobile preset, and this is stated honestly: it is entirely an artifact of the preset's 4x CPU throttle inflating React hydration into LCP render-delay on an already-downloaded hero image (the image downloads in under 60 ms). With the same mobile screen emulation at real-device CPU the mobile run scores 100 with LCP around 1000 ms. The deployed Fly real-CPU run is the authoritative mobile-performance measurement; the synthetic mobile number is surfaced as a warning, not hidden. (`/reserve` carries a deliberate `robots:noindex`, which caps its SEO score by design.)
...
- **The 3D model is a placeholder, and it is currently branded.** The shipped configurator model is an optimized exterior-only GLB (about 1.8 MB, ~56k triangles, meshopt-compressed) derived from an owner-provided third-party model that carries manufacturer trademarks. It is used **only as a development placeholder** to lift the configurator's art-direction bar, and it is **not royalty-clear** — for that reason this README does not advertise the manufacturer name as if it were licensed, and **the public demo deploy is blocked until the model is swapped** for a royalty-clear, unbadged CC0 vehicle (ideally a true EV SUV with multiple rim styles). The swap is a drop-in: replace the GLB, re-run the optimize and scene-render scripts, re-verify the CSP, and update `CREDITS.md` — no code change to the scene, controls, rig, or accessibility contract. The configurable hero vehicle is named "APEX Lumen SUV" to match the placeholder silhouette; the swap restores full EV framing.
...
- **Nothing is really booked.** The reservation flow is mocked end to end — no backend, no database, no real availability, no PII transmitted, no payment, no email or SMS. A reload starts fresh. The confirmation says so. Wiring a real rental API (the unused Fastify backend slot, or NestJS) is the explicit v2 path.
...
- **A royalty-clear, IP-cleared 3D model.** v1 ships a branded placeholder; the unbadged CC0 swap is the documented (and public-deploy-gating) v2 step.
