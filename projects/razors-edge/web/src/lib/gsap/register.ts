...
/**
 * Lazy GSAP loader (ADR-002, Home-Perf pass 2026-06-03).
 *
 ...
 *
 * This module therefore no longer STATICALLY re-exports `gsap` (which pulled
 * the whole library into the home route's initial bundle). Instead it
 * exposes an async `loadGsap()` that dynamically `import()`s the three
 * modules, registers the plugins exactly once, and resolves the handles —
 * so the GSAP chunk is code-split out of the initial bundle and fetched
 * AFTER hydration / on idle (see `useGsapEffect`). The static markup is
 * untouched and remains the floor; GSAP only attaches on top.
 *
 ...
 * `registerPlugin` call. ScrollTrigger is still never imported into a Server
 * Component — these are dynamic imports inside client-only code paths.
 *
 * CSP (ADR-002): GSAP core + ScrollTrigger + `@gsap/react` do NOT use
 * `eval` / `new Function`, so this runs clean under `script-src 'self'`
 * with no `'unsafe-eval'`. GSAP writes inline `style` transforms during
 * scrubs, which `style-src 'unsafe-inline'` covers. Verified under
