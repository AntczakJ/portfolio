...
**Live: [nocturne-demo.fly.dev](https://nocturne-demo.fly.dev)** (Fly.io, single Machine, Next.js standalone, region `fra`, no backend, no secrets, scale-to-zero — the `atrium`/`apex` web-only standalone pattern; deployed 2026-06-14; see [DECISIONS.md](./DECISIONS.md) ADR-005 and [DEPLOY.md](./DEPLOY.md)). The GPU particle field + Web Audio run client-side in your browser, so the experience wants a real GPU — the Fly Machine only serves the bundle. The canonical origin is wired through `NEXT_PUBLIC_SITE_URL`, baked at build time, and backs the sitemap, canonical tags, the Open Graph image, and the JSON-LD. You can also run it locally per [Run locally](#run-locally).
