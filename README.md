...
| [`atrium`](./projects/atrium/)           | The portfolio's front door: a scroll-driven cinematic landing page that descends through the `ATRIUM` wordmark into a colonnade of light, then glides through six kinetic gallery bays — one per project, each lit in its own signature hue — into a fully-legible directory. | Next 15 + React 19 + Tailwind v4 + GSAP (scroll) + Zod (web-only, no backend)                                                            | local / not yet deployed                                     |

## Philosophy

Seven projects. Four are api-heavy, deliberately spread across four distinct backends — Elysia/Bun (`tape`), Hono/Node (`meld`), NestJS/Node (`pulse`), and Fastify/Node (`atlas`) — so the backend-variance story is complete rather than one framework repeated; all four are deployed. The other three (`razors-edge`, `apex`, `atrium`) are web-only design showcases: a cinematic marketing site, a genuine WebGL 3D configurator, and the portfolio's own landing page — a scroll-driven lobby that frames the whole collection. `atrium` is the front door (built, not yet deployed); the other six are live.
