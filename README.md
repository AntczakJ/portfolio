...
| [`apex`](./projects/apex/)               | Premium car-rental marketing site with a real WebGL 3D car configurator (live colour + wheel swaps, four-tier degradation) and a fully mocked multi-step reservation flow.                                                                                                                                                        | Next 15 + React 19 + Tailwind v4 + React Three Fiber + drei (3D) + GSAP (scroll) + TanStack Query + Zustand + Zod (web-only, no backend) | [apex-rentals.fly.dev](https://apex-rentals.fly.dev)         |
| [`atlas`](./projects/atlas/)             | Live geospatial fleet tracking: a server-side deterministic simulation streams fleet telemetry over one WebSocket to a MapLibre control-room map that glides the vehicles at 60 fps between 1 Hz ticks, with live ETAs and geofence events.                                                                                       | Next 15 + React 19 + Tailwind v4 + MapLibre + Motion + Fastify (Node 22) + @fastify/websocket + turf + Postgres + Drizzle (api-heavy)    | [atlas-ops.fly.dev](https://atlas-ops.fly.dev)               |
| [`atrium`](./projects/atrium/)           | The portfolio's front door: a scroll-driven cinematic landing page that descends through the `ATRIUM` wordmark into a colonnade of light, then glides through six kinetic gallery bays — one per project, each lit in its own signature hue — into a fully-legible directory.                                                     | Next 15 + React 19 + Tailwind v4 + GSAP (scroll) + Zod (web-only, no backend)                                                            | [atrium-demo.fly.dev](https://atrium-demo.fly.dev)           |
| [`nocturne`](./projects/nocturne/)       | A GPU audio-reactive generative particle experience: a field of ~262k particles flowing through an animated curl-noise field, computed on the GPU via FBO ping-pong, breathing in real time with the music, re-skinnable by curated presets, finished with cinematic bloom/vignette/aberration in a fullscreen sovereign-dark UI. | Next 16 + React 19 + Tailwind v4 + R3F + drei + postprocessing + custom GLSL + Web Audio (web-only, no backend)                          | deploy pending                                               |

## Philosophy

Eight projects. Four are api-heavy, deliberately spread across four distinct backends — Elysia/Bun (`tape`), Hono/Node (`meld`), NestJS/Node (`pulse`), and Fastify/Node (`atlas`) — so the backend-variance story is complete rather than one framework repeated; all four are deployed. The other four are web-only creative showcases: a cinematic marketing site (`razors-edge`), a genuine WebGL 3D configurator (`apex`), the portfolio's own scroll-driven landing page (`atrium`), and a GPU audio-reactive generative particle experience (`nocturne`). `nocturne` is the second R3F project, distinct from `apex` at the opposite end of the WebGL spectrum — generative GPGPU art that breathes with sound, not a product configurator. `atrium` is the front door — the deployed projects are live, with `nocturne`'s deploy pending.
